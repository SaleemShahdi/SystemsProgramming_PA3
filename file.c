...
static int print_file_info(const char *pathname, const struct stat *statbuf,
    ...
    printf("Inode: %lu, Name: %s\n", (unsigned long)statbuf->st_ino, pathname);
    printf("typeflag: %d\n", typeflag);
    switch (typeflag)
    {
    case FTW_F:
    ...
    { // Only check regular files and directories
...
int main(int argc, char *argv[])
{
    ...
    if (nftw(argv[1], print_file_info, 20, FTW_PHYS) == -1) // last 2 values passed into nftw used to be 20 and 0
    {
        perror("nftw");
        exit(EXIT_FAILURE);
    }
